...
void PATTERN::setupPatternFile()
{
	...
	*/
	if(!ofs.is_open())
		ofs.open(pattern_name.c_str(), ofstream::out | ofstream::app);   

	if(!ofs.is_open())                                                         
		std::cout << "Cannot open file!\n";
	else
		std::cout << "Successfully open file: " << pattern_name << endl;
}
...
void PATTERN::setPatterninput()
{
	...
	if(!patterninput.is_open()){
		cout << "Unable to open pattern file: " << pattern_name << endl;
		exit(-1);
	}
...
void PATTERN::genRandomPatternOnly(int pattern_number)
{
	int i, j, random(0);
